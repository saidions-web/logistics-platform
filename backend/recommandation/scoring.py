...
from tarif.models import Tarif   # ⚠️ IMPORTANT : depuis app tarif
...
def get_eligibles(commande):
    ...
    for e in entreprises:
        ...
        # ⚠️ Correction ici (robuste string / JSON)
        if gouvernorat.lower() not in str(zones).lower():
            continue
...
def generer_recommandation(commande):
    ...
    # ✅ FIX PRO : ne pas écraser le choix manuel
    if not reco.selection_manuelle:
        reco.entreprise_recommandee = best

        # ⚠️ important : ne pas écraser si déjà choisi
    ...
    if best:
        Notification.objects.create(
            ...
            message=f"Commande {commande.reference} assignée"
        )
